...
def test_create(new_db: DataBase) -> None:
    ...
    with pytest.raises(ValueError):
        students.delete_record(key=1000111)
...
def test_update(new_db: DataBase) -> None:
    ...
    add_student(students, 111, Birthday=dt.datetime(1995, 4, 28))
    assert students.count() == 1
    students.update_record('1000111', dict(First='Jane', Last='Doe'))
    assert students.get_record('1000111')['First'] == 'Jane'
...
def test_50_students(new_db: DataBase) -> None:
    ...
    students.delete_record(1000001)
    students.delete_records([SelectionCriteria('ID', '=', 1000020)])
    students.delete_records([SelectionCriteria('ID', '<', 1000003)])
    students.delete_records([SelectionCriteria('ID', '>', 1000033)])
    students.delete_records([
        SelectionCriteria('ID', '>', 1000020),
        SelectionCriteria('ID', '<', 1000023)
    ])
    assert students.count() == 28
    students.update_record(1000009, dict(First='Jane', Last='Doe'))
